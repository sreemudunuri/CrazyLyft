...
function bloodComponent(width, height, x, y) {
...
function checkAllObsticals() {
     return obstacles.some(obst => myGamePiece.crashWith(obst)
     );
}
...
function addBlood(passa, ) {
     blood.push(new bloodComponent(25, 25, passa.x, passa.y))
     setTimeout(() => { blood = [] }, 2000)
}

function checkAllPassangers() {
     if (myGamePiece.crashWith(passangersArray[0])) {
          addBlood(passangersArray[0], dir)
...
function playerMove(e) {
     // key W
     if (keys[87] || keys[38] && !checkAllObsticals() && myGamePiece.y > 2) {
          dir = 180
          myGamePiece.y -= speed;
     }
     if (keys[87] || keys[38] && checkAllObsticals() && myGamePiece.y > 2) {
     ...
     if (keys[83] || keys[40]&& !checkAllObsticals() && myGamePiece.y < 800 - myGamePiece.height - 2) {
          dir = 0
          myGamePiece.y += speed;
     }
     if (keys[83] || keys[40]&& checkAllObsticals() && myGamePiece.y < 800 - myGamePiece.height - 2) {
     ...
     if (keys[65] || keys[37]&& !checkAllObsticals() && myGamePiece.x > 2) {
          dir = 90
          myGamePiece.x -= speed;
     }
     if (keys[65] || keys[37]&& checkAllObsticals() && myGamePiece.x > 2) {
     ...
     if (keys[68] || keys[39] && !checkAllObsticals() && myGamePiece.x < 1200 - myGamePiece.width - 2) {
          dir = 270
          myGamePiece.x += speed;
     }
     if (keys[68] || keys[39] && checkAllObsticals() && myGamePiece.x < 1200 - myGamePiece.width - 2) {
